...
def git_hash_all_code():
    ...
    if is_dirty:
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            with open(os.path.join(ROOT, ".git", "index"), "r") as git_index:
            ...
            if output.returncode != 0:
                raise Exception(
                    "failed to get working tree hash (`git add`).\n"
                    f"stdout: {output.stdout}\n"
                    f"stderr: {output.stderr}\n"
                )
            ...
            if output.returncode != 0:
                raise Exception(
                    "failed to get working tree hash (`git write-tree`).\n"
                    f"stdout: {output.stdout}\n"
                    f"stderr: {output.stderr}\n"
                )

            short_hash = output.stdout.decode("utf8")[:7]
    else:
        output = subprocess.run(
            ["git", "rev-parse", "HEAD"],
            capture_output=True,
        ...
        if output.returncode != 0:
            raise Exception(
                "failed to get HEAD hash (`git rev-parse HEAD`).\n"
                f"stdout: {output.stdout}\n"
                f"stderr: {output.stderr}\n"
            )
        short_hash = output.stdout.decode("utf8")[:7]

    return ("dirty." if is_dirty else "") + short_hash
