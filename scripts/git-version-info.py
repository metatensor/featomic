...
def git_last_mtime():
    output = run_subprocess(["git", "ls-files", ROOT])

    git_files = output.stdout.strip().split("\n")
    last_modification = 0
    for path in git_files:
        # skip files that where removed but not yet committed
        if os.path.exists(path):
            timestamp = os.path.getmtime(path)
            if timestamp > last_modification:
                last_modification = timestamp

    last_modification = datetime.datetime.fromtimestamp(last_modification)
    return last_modification.strftime("%Y%m%d%H%M%S")
...
if __name__ == "__main__":
    ...
    if n_commits_since_last_tag(tag_prefix) == 0:
        print(0)
    else:
        print(git_last_mtime())
