...
FEATOMIC_PYTHON_SRC = (ROOT / ".." / "featomic").resolve()
FEATOMIC_TORCH_SRC = (ROOT / ".." / ".." / "featomic-torch").resolve()
...
class cmake_ext(build_ext):
    ...
    def run(self):
        ...
        install_dir = pathlib.Path(self.build_lib).resolve() / "featomic" / "torch"
...
def generate_cxx_tar():
    script = ROOT / ".." / ".." / "scripts" / "package-featomic-torch.sh"
...
if __name__ == "__main__":
    ...
    if not FEATOMIC_TORCH_SRC.exists():
        # we are building from a sdist, which should include featomic-torch
        # sources as a tarball
        tarballs = glob.glob("featomic-torch-cxx-*.tar.gz", root_dir=ROOT)
        ...
        FEATOMIC_TORCH_SRC = pathlib.Path(tarballs[0]).resolve()
        subprocess.run(
        ...
        FEATOMIC_TORCH_SRC = pathlib.Path(
            ".".join(str(FEATOMIC_TORCH_SRC).split(".")[:-2])
        )

    with open(FEATOMIC_TORCH_SRC / "VERSION") as fd:
    ...
    if not FEATOMIC_NO_LOCAL_DEPS and FEATOMIC_PYTHON_SRC.exists():
